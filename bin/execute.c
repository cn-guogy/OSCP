...
/* 条件码计算 */
#define b31(z) (((z) >> 31) & 0x1) /* 提取第31位 */

/* 代码看起来很奇怪，但由于MIPS的原因，它是快速的！ */
...
char *argv[];
{
	...
	pc = startpc;
	npc = pc + 4;
	i = MEMSIZE - 1024 + memoffset; /* 初始SP值 */
	Reg[29] = i;					/* 初始化SP  */
	/* 设置argc和argv的内容（麻烦！） */
	store(i, argc);
	aci = i + 4;
