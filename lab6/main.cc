...
//    ThreadTest();
	...
	for (argc--, argv++; argc > 0; argc -= argCount, argv += argCount)
	{
		...
		{ // 测试控制台
			...
			interrupt->Halt(); // 一旦我们启动控制台，Nachos 将永远循环等待
							   // 控制台输入
		}
#endif // USER_PROGRAM
#ifdef FILESYS
	...
	currentThread->Finish(); // 注意: 如果 "main" 过程返回，
							 // 那么程序 "nachos" 将退出（就像其他正常程序一样）。
							 // 但可能还有其他线程在就绪列表中。
							 // 我们通过声明 "main" 线程已完成来切换到这些线程，
							 // 防止它返回。
	return (0); // 不会到达...
}
